...
```sh
uv run python -m curtain_ml.training \
  --epochs 1 \
  --batch-size 8 \
  --workers 0 \
  --limit 32 \
...
```sh
modal run --detach cloud/train.py \
  --epochs 20 \
...
```sh
uv run python -m scripts.select_frames --total 50000
...
```sh
.venv/bin/modal run cloud/index_collection.py
```
...
modal run cloud/index_movie.py --movie-name The_Matrix_1999
...
```sh
uv run python -m scripts.search screenshots/The_Matrix_1999/frame_000001.jpg \
...
`.venv/bin/modal run cloud/benchmark_crops.py`. It compares original,
...
```sh
.venv/bin/modal run cloud/benchmark_lighting.py
```
...
```sh
.venv/bin/python -m unittest tests.test_augmentations tests.test_lighting tests.test_retrieval
```

`curtain_ml/augmentations.py` provides `LightingConfig` and `RandomLighting` using
...
Use `--lighting-preset lighting-v1` with either `python -m curtain_ml.training` or
`modal run cloud/train.py`. The default `legacy` recipe is unchanged,
...
```sh
.venv/bin/python -m unittest tests.test_lighting tests.test_retrieval
```

Run the fixed-seed, 314-source-frame diagnostic against both full collections:

```sh
OPENBLAS_NUM_THREADS=4 OMP_NUM_THREADS=4 .venv/bin/python -m benchmarks.collection_retrieval --normalize
.venv/bin/python -m benchmarks.audit_failures
